...
# studetn upvote for firstround
  def upvote
    ...
    if !vote_wich_exist.empty?
      ...
      render :json =>  {delete: "You just deleted vote for project <strong>#{pitch.title}</strong>.", votes: votes}
    else
      if (current_user.votes.count >= current_user.cohort.setting.student_vote_first_round)
        ...
        votes = current_user.votes.count
        render :json =>  {count: current_user.votes.count, message: "You just vote on project <strong>#{pitch.title}</strong>.", votes: votes}
      end
    end
  end
